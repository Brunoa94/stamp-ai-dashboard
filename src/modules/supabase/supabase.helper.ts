...
export function appendInvoicesFilters(filters: GetInvoicesFiltersType) {
  ...
  if (filters.issued_from) {
    query = query.gte("issued_at", filters.issued_from);
  }

  if (filters.issued_to) {
    query = query.lte("issued_at", filters.issued_to);
  }
